// focus/src/pages/fragment/beranda/DaftarAktivitasSectionHead.jsx
...
const STORAGE_KEY = 'focus_selected_activity_sectionhead';

const DaftarAktivitasSectionHead = () => {
  const { user, userData } = useAuth();
  const [loading, setLoading] = useState(true);
  const [tabs, setTabs] = useState([]);
  const [selectedTab, setSelectedTab] = useState(null);
  const [allIdentitas, setAllIdentitas] = useState([]);
  const [sectionHeadStepId, setSectionHeadStepId] = useState(null);
  const [sectionHeadAssignedUsers, setSectionHeadAssignedUsers] = useState([]);
  const [sectionHeadLogsMap, setSectionHeadLogsMap] = useState({});
  ...
  // 🔥 Inisialisasi default: pilih tab pertama setelah data siap
  useEffect(() => {
  ...
  const fetchActivities = async () => {
    ...
    try {
      ...
      // 🔥 Cari step "Approval Section Head" yang aktif
      ...
      Object.values(stepsData).forEach(step => {
        if (
          ...
          step.nama_step &&
          step.nama_step.toLowerCase() === 'approval section head'
        ) {
          stepId = step.id_workflow_approval_steps;
          // Konversi assigned_user_ids
      ...
      setSectionHeadStepId(stepId);
      setSectionHeadAssignedUsers(assignedUsers);

      // 🔥 Ambil log approval untuk step "Approval Section Head" dan assigned_to_user_id = uid
      ...
      Object.values(logData).forEach(log => {
        if (log.workflow_step_id === stepId && log.assigned_to_user_id === uid) {
          ...
          // Simpan log terbaru untuk setiap dataId
          if (!logMap[dataId] || new Date(log.created_at) > new Date(logMap[dataId].created_at)) {
            logMap[dataId] = log;
          }
        }
      });
      setSectionHeadLogsMap(logMap);

      // Reset selected (akan diisi oleh useEffect default)
      setSelectedTab(null);
  ...
  const computeStatusCounts = (daftarId) => {
    ...
    allIdentitas.forEach(item => {
      if (item.daftar_aktivitas_id === daftarId) {
        const log = sectionHeadLogsMap[item.id_data_identitas_aktivitas];
        ...
        if (log) {
          ...
          if (currentStepId === sectionHeadStepId && sectionHeadAssignedUsers.includes(uid)) {
            counts.pending += 1;
          }
        }
      }
...
export default DaftarAktivitasSectionHead;